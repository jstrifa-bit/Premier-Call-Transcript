...
SOP RECOMMENDATIONS:
Each rule below declares the flags it depends on (its required_flags). A rule fires ONLY when (a) ALL of its required_flags have NON-NULL values AND (b) its trigger condition is satisfied by an EXPLICIT, NON-NULL flag value. NEVER treat a null flag as "false" or "absent" - if the transcript did not address a required_flag, the rule does NOT fire. A routing recommendation generated from missing data is more dangerous than no recommendation at all.

Trigger conditions (each requires the named flag to be EXPLICITLY set to the stated value - not null, not inferred from silence):
- GEN-001 (General, Action Required, priority 7): dental_visit_within_6_months explicitly == false OR pending_dental_work explicitly == true.
- JNT-001 (Joint, Deferred, priority 2): active_smoker explicitly == true OR quit_smoking_within_3_months explicitly == true.
- JNT-002 (Joint, Ineligible, priority 1): attempted_pt explicitly == false (i.e. the transcript states the patient has NOT attempted PT). If attempted_pt is null because PT was never discussed, do NOT fire JNT-002.
- JNT-003 (Joint, Review, priority 4): hba1c_value is a non-null number greater than 7.0.
- JNT-004 (Joint, High Complexity, priority 3): daily_opioid_use explicitly == true AND opioid_duration_months explicitly > 3.
- BAR-001 (Bariatric, Revision Case, priority 5): prior_weight_loss_surgery explicitly == true.
- BAR-002 (Bariatric, Action Required, priority 7): endoscopy_within_3_months explicitly == false.
- BAR-003 (Bariatric, Hold, priority 6): has_registered_dietician explicitly == false.

INCOMPLETE TRANSCRIPT RULE:
If extraction_metadata.extraction_confidence is "low" OR extraction_metadata.null_flag_count > 3, output an EMPTY sop_recommendations array and an EMPTY next_steps array. Set extraction_metadata.requires_human_review = true with a review_reason like "Insufficient information to generate SOP recommendations safely." Set overall_case_status to "Hold". Deferring routing is safer than generating recommendations from incomplete data.

For each triggered SOP that fires, fill: id, category, finding (one short sentence describing the trigger), case_status, action, triggered_by_flags (the flag names that fired it), priority, confidence (mirror the confidence of the weakest triggering flag).
...
${schemaText}`;
  ...
  // DEBUG: dump the raw model response so we can diagnose extraction issues
  // from the Vercel function logs (Project -> Logs). Remove this block once
  // extraction output is stable.
  console.log("RAW EXTRACT RESPONSE (first 500 chars):");
  ...
  try {
    ...
    console.warn("EXTRACT PARSE FAILURE:", parseErr.message);
    console.warn("CLEANED PAYLOAD (first 1000 chars):");
    console.warn(cleaned.slice(0, 1000));
