...
function buildLocalSummary(crm, findings, transcript) {
  ...
  if (findings.length > 0) {
    ...
    out.push("Key clinical points raised in the call: " + topics.join("; ") + ".");
  } else {
    out.push("No SOP triggers were detected in this call.");
  }
  const a1c = findA1c(transcript);
  if (a1c) out.push(`Most recent HbA1c referenced: ${a1c}.`);
  if (findings.length >= 2) {
    out.push("Multiple workup gaps were identified - see the SOP findings below for the full list.");
  } else {
...
function buildLocalNextSteps(findings, disposition) {
  ...
  const tails = {
    "Ineligible": "Notify patient of ineligibility and close case in EHR.",
  ...
  steps.push("Document the call summary and disposition in the patient's EHR record.");
  return steps;
}
